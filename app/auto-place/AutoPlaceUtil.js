 ...
 * @return {Point}
 */
export function getNewShapePosition(source, element, elementDetection) {

  if (is(element, 'bpmn:TextAnnotation')) {
    return getTextAnnotationPosition(source, element, elementDetection);
  }

  if (isAny(element, [ 'bpmn:DataObjectReference', 'bpmn:DataStoreReference' ])) {
    return getDataElementPosition(source, element, elementDetection);
  }

  if (is(element, 'bpmn:FlowNode')) {
    return getFlowNodePosition(source, element, elementDetection);
  }
 ...
 */
export function getFlowNodePosition(source, element, elementDetection) {
  ...
  return deconflictPosition(source, element, position, escapeDirection, elementDetection);
}
 ...
 */
export function getTextAnnotationPosition(source, element, elementDetection) {
 ...
 */
export function getDataElementPosition(source, element, elementDetection) {
 ...
 * @return {Point}
 */
export function deconflictPosition(source, element, position, escapeDelta, elementDetection) {
  ...
  var detectedElements, existingTarget;

  // deconflict position until free slot is found
  do {
    var detectedElements = elementDetection.detectAt({
      x: position.x - element.width / 2,
      y: position.y - element.height / 2,
      width: element.width,
      height: element.height
    ...
    if (detectedElements.length) {
      existingTarget = detectedElements[0];
